...
import { useListenFeedSubscription } from '~/graphql/hooks/Subscriptions';
...
type AuthContextState = {
    ...
    feedCount: number;
    refetchFeedCount: (count?: number) => void;
    loaded: boolean;
    isGuest: boolean;
...
export function AuthProvider({ children }: AuthProviderProps): JSX.Element {
    ...
    const { data: listenFeedData } = useListenFeedSubscription({
        skip: !userState?.userId,
        variables: { target: userState?.userId || '' },
        onSubscriptionData: (options) => {
            if (options?.subscriptionData?.data?.listenFeed?.id) {
                ...
                try {
                    ...
                    if (response.data?.meEx) {
                        ...
                        if (member?.id === storageUser.memberId && member?.address === storageUser.address) {
                            ...
                            setFeedCount(response.data?.feedsStatus?.notReadCount ?? 0);
        ...
        async (keepSession: boolean): Promise<LoginResultType> => {
            ...
            try {
                ...
                if (response.data?.signInMember) {
                    const { user, jwt, feeds } = response.data.signInMember;
                    if (!jwt) {
                        return {
                            succeeded: false,
                    ...
                    setFeedCount(feeds?.notReadCount ?? 0);
    ...
    const refetchFeedCount = useCallback((count?: number) => {
