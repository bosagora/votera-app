...
function VoteScreen(props: Props): JSX.Element {
    ...
    switch (voteProposalState) {
        case EnumVoteProposalState.None:
            return (
                <View onLayout={(event) => onLayout(event.nativeEvent.layout.height + 50)}>
                    <PendingVote proposal={proposal} />
                </View>
            );
        case EnumVoteProposalState.Running:
            if (proposal?.status === EnumProposalStatus.PendingVote || !isValidator || isGuest) {
                return (
                    <View onLayout={(event) => onLayout(event.nativeEvent.layout.height + 50)}>
                        <PendingVote proposal={proposal} />
                    </View>
            ...
            return (
                <View onLayout={(event) => onLayout(event.nativeEvent.layout.height + 50)}>
                    <Voting runVote={runVote} canVote={isValidator && !isGuest} needVote={needVote} />
                </View>
            );
        default:
            break;
